...
exports.main = async () => {
  try {
    ...
    const visibleList = normalizedList
      .filter((item) => ['published', 'closed'].includes(normalizeText(item && item.status)))
      .sort((a, b) => {
        const getOrder = (activity) => {
          if (isActivityRunning(activity)) {
            return 0
          }

          if (normalizeText(activity && activity.status) === 'published') {
            return 1
          }
        ...
        const statusDiff = getOrder(a) - getOrder(b)
    ...
    return {
      ...
      list: visibleList,
      stats: {
        totalActivities: visibleList.length,
        ongoingActivities: normalizedList.filter((item) => isActivityRunning(item)).length,
        totalChars: visibleList.reduce((sum, item) => sum + (Number(item && item.filledCount) || 0), 0)
      }
    }
  } catch (error) {
    ...
    return {
      success: false,
      message: error.message || '诗词摊煎饼活动加载失败'
    }
  }
}
